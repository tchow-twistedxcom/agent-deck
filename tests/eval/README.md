...
GOTOOLCHAIN=go1.25.10 go test -tags eval_smoke \
...
GOTOOLCHAIN=go1.25.10 go test -tags 'eval_smoke eval_full' \
  ./tests/eval/... ./internal/ui/...

# Single case, verbose.
GOTOOLCHAIN=go1.25.10 go test -tags eval_smoke -v \
  -run TestEval_FeedbackCLI_DisclosureBeforeConsent ./tests/eval/feedback/...
```

`GOTOOLCHAIN=go1.25.10` matches the toolchain pinned in `go.mod` and
