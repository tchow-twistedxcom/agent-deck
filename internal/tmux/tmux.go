...
func (s *Session) Start(command string) error {
	...
	// reader (e.g. iTerm2 on macOS).
	//
	// Fork gate (6bd1ec93): only bind when socket isolation is active. The
	// root-table bind is server-global — on a shared default tmux server it
	// eats Ctrl+Q from ALL sessions, including the user's own wrapper session
	// when agent-deck runs NESTED inside tmux, consuming the byte before the
	// PTY attach loop's IndexDetachKey handler can see it. Binding only when
	// SocketName is set confines it to a private -L server where every session
	// is agent-deck-managed. See detach_bind_gate_test.go.
	//
	// The binding itself uses upstream's if-shell -F form via
	// ctrlQDetachBindArgs():
	//
	// #1808: if-shell -F re-evaluates a tmux FORMAT against the CURRENT
	// client's session at keypress time instead of baking in one session name,
	// so the detach scopes to "whichever agentdeck session this client is
	// attached to" rather than "the one session that happened to Start() last".
	//
	// #1820: the pattern (#{m:pattern,string}) is evaluated entirely inside
	// tmux's own format engine — nothing is handed to a shell, so a session
	// name containing a quote cannot break out and run commands — and
	// "detach-client" runs directly on the invoking client's own command queue
	// (no run-shell subprocess), so it detaches exactly the client that pressed
	// the key even when the session has more than one client attached (e.g. a
	// web `tmux -C` control client alongside a native terminal client; see
	// controlpipe.go). Bounded — see tmuxMutationTimeout.
	if s.SocketName != "" {
...
func (s *Session) EnableMouseMode() error {
	...
	// user tmux setting wins (mirrors Start; see gatedTmuxKeyOptionArgs). The
	// gated helper still sets extended-keys-format csi-u so Shift+Enter reaches
	// Claude Code as ESC[13;2u (the fork's csi-u fix, now absorbed upstream).
